...
```
--
Aquí nos adelantamos un poco con el contenido, pero el siguiente código se puede leer como "del dataset hospital, obtén las 3 primeras  filas, luego selecciona las columnas Numero y Genero y finalmente mantén solo las filas cuyo Genero es 'Male'". Esto resulta más fácil de
leer que la sintaxis alternativa sin uso de _pipes_:

```{r, eval = F}
filter(select(slice_head(hospital, n = 3), Numero, Genero), Genero == 'Male')
```
...
```{r, warning=FALSE, eval=FALSE, echo = T}
hospital %>% filter(Genero == 'Female' & (Ciudad == 'Boston' | Ciudad == 'Dallas'))

```
--
```{r, warning=FALSE, eval=TRUE, echo=F}
knitr::kable(hospital %>% filter(Genero == 'Female' &
                                   (Ciudad == 'Boston' |
